...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-scheduling-service ./cmd/scheduling-service
...
COPY --from=builder /app/bin/sentiric-scheduling-service .
RUN chown appuser:appgroup ./sentiric-scheduling-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-scheduling-service"]